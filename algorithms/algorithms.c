...
void	ft_sort_seven(t_list **stack_a, t_list **stack_b)
{
	...
	while (ft_is_sorted(*stack_a) || ft_lstsize(*stack_a) < i)
	{
		...
		if (!ft_is_sorted(*stack_a) && *stack_b)
		{
			...
			sa(stack_a);
		else if (ft_lstdistance(stack_a, ft_smallest_node(*stack_a)) > 3)
			rra(stack_a);
...
void	ft_sort_up(t_list **stack_a, t_list **stack_b)
{
	while(ft_is_sorted(*stack_a))
	{
		...
		if (ft_lstsize(*stack_b) == 5)
			ft_reverse_sort_five(stack_a, stack_b);
		ft_with_a(stack_a, stack_b);
		ft_with_b(stack_a, stack_b);
	}
	while (ft_lstsize(*stack_b) > 5 && (*stack_a)->next->content < ft_biggerst_node(*stack_b)->content)
		pb(stack_a, stack_b);
...
void	ft_with_b(t_list **stack_a, t_list **stack_b)
{
	if (ft_lstsize(*stack_b) > 5 && ft_reverse_sorted(*stack_b))
	{
...
// void	ft_after_half(t_list **stack_a, t_list **stack_b)
// {
